...
import { type DefenseAnswer, type NextTurn, type PlayerInGameStatusArray, type PlayerStatus } from './types/game'
...
mainSocket.on('connection', (socket: Socket) => {
  ...
  socket.on('defAns', async (data: DefenseAnswer) => {
    try {
      ...
      const playerStatus = await get(playerStatusRef).then((snap: any) => snap.val())
      // Defending
      if (data.ans) {
  ...
  socket.on('turnEnd', async (data: NextTurn) => {
    try {
      ...
      if (playerStatus[nextPlayerId].dead === true) {
